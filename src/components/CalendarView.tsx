    ...
    <div className="p-3 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center space-x-3">
            <div className="p-2.5 bg-blue-600/10 border border-blue-500/20 text-blue-600 rounded-xl">
              <CalendarIcon className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Compliance Workload Calendar</h1>
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          ...
          <div className="flex items-center bg-white border border-slate-200 rounded-xl shadow-sm p-1">
            ...
            <span className="font-bold text-slate-800 text-sm px-3 select-none min-w-[140px] text-center">
              {monthName}
            </span>
            <button
          ...
          <button
            onClick={handleToday}
            className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 font-semibold text-xs px-3 py-2 rounded-xl shadow-sm transition-all cursor-pointer"
          >
            Current Month
          </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Total Deadlines</p>
            <p className="text-2xl font-black text-slate-900 mt-1">{totalEventsCount}</p>
          </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-emerald-600 uppercase tracking-wider">Filed / Paid</p>
            <p className="text-2xl font-black text-emerald-600 mt-1">{completedEventsCount}</p>
          </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-black text-amber-600 mt-1">{pendingEventsCount}</p>
          </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-[11px] font-semibold text-red-600 uppercase tracking-wider">Overdue</p>
            <p className="text-2xl font-black text-red-600 mt-1">{overdueEventsCount}</p>
          </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-slate-50 border border-slate-200 text-slate-800 placeholder-slate-400 text-xs rounded-lg pl-9 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center space-x-1.5 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs">
            ...
            <select
              ...
              className="bg-transparent text-slate-700 font-medium focus:outline-none cursor-pointer max-w-[160px] truncate"
            >
      ...
      {viewMode === 'grid' ? (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          {/* Day of Week Headers */}
          <div className="grid grid-cols-7 bg-slate-100 border-b border-slate-200 text-center py-2.5 text-xs font-bold text-slate-600 uppercase tracking-wider">
          ...
          <div className="grid grid-cols-7 auto-rows-fr divide-x divide-y divide-slate-100 bg-slate-50">
            ...
            {Array.from({ length: firstDayOfWeek }).map((_, i) => (
              <div key={`empty-prev-${i}`} className="min-h-[110px] bg-slate-100/50 p-1.5 opacity-40 select-none" />
            ))}
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  ...
                  className={`min-h-[110px] p-1.5 sm:p-2 bg-white transition-all flex flex-col justify-between group ${
                    dayEvents.length > 0 ? 'hover:bg-blue-50/30 cursor-pointer' : ''
                  } ${isToday ? 'bg-amber-50/40 ring-2 ring-amber-400/80 inset-0 z-10' : ''}`}
                >
                  {/* Top Bar of Day Box */}
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {dayEvents.length > 0 && (
                      <span className="text-[10px] font-extrabold text-slate-500 bg-slate-100 px-1.5 py-0.2 rounded-full">
                  ...
                  <div className="space-y-1 flex-1 overflow-hidden">
                    ...
                    {dayEvents.length > 3 && (
                      <div className="text-[10px] font-bold text-blue-600 text-center py-0.5 hover:underline">
        ...
        <div className="space-y-4">
          {agendaDays.length === 0 ? (
            <div className="bg-white rounded-2xl p-12 border border-slate-200 text-center">
              ...
              <h3 className="text-base font-bold text-slate-800">No BIR Deadlines Found</h3>
            ...
            agendaDays.map(dayNum => {
              ...
              return (
                <div key={`agenda-day-${dayNum}`} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                  {/* Date Group Header */}
                  <div className={`p-4 border-b border-slate-100 flex items-center justify-between ${
                    isToday ? 'bg-amber-500/10 border-amber-200' : 'bg-slate-50/80'
                  }`}>
                    <div className="flex items-center space-x-2.5">
                      ...
                      <div>
                        <h2 className="text-sm font-bold text-slate-900">{dateFormatted}</h2>
                        {isToday && <span className="text-[10px] font-extrabold text-amber-700 uppercase">TODAY</span>}
                      </div>
                    </div>
                  ...
                  <div className="divide-y divide-slate-100">
                    {dayEvents.map(event => {
                      ...
                      return (
                        <div key={`${event.clientId}-${event.form.id || event.form.code}`} className="p-4 hover:bg-slate-50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex items-start space-x-3.5">
                            ...
                            <div>
                              <div className="flex flex-wrap items-center gap-2">
                                <h3 className="text-sm font-bold text-slate-900">{event.clientName}</h3>
                              ...
                              <p className="text-xs text-slate-600 mt-0.5">{event.form.description}</p>
                          ...
                          <div className="flex items-center space-x-2 shrink-0">
                            {!isFiled && (
                              <>
                                <button
                                  ...
                                  className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors flex items-center space-x-1 cursor-pointer"
                            ...
                            {isFiled && (
                              <span className="text-xs font-bold text-emerald-600 flex items-center space-x-1 bg-emerald-50 px-3 py-1.5 rounded-lg">
                  ...
                  <div key={`${event.clientId}-${event.form.id || event.form.code}`} className="py-3.5 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="font-extrabold text-blue-600 text-sm">BIR Form {event.form.code}</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${
                          isFiled ? 'bg-emerald-100 text-emerald-800' : isOverdue ? 'bg-red-600 text-white' : 'bg-amber-500 text-white'
                        }`}>
                          {event.form.status}
                        </span>
                      </div>
                      <p className="font-bold text-slate-900 text-xs mt-0.5">{event.clientName}</p>
                      <p className="text-xs text-slate-500 mt-0.5">{event.form.description}</p>
                    </div>
                    ...
                    <div className="flex items-center space-x-2 shrink-0">
                      {!isFiled && (
                        <>
                          <button
                            onClick={() => handleSendReminder(event)}
                            className="bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold px-2.5 py-1.5 rounded-lg transition-colors flex items-center space-x-1"
                          >
                          ...
                          <button
                            onClick={() => handleMarkFiled(event)}
                            className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-2.5 py-1.5 rounded-lg transition-colors flex items-center space-x-1"
                          >
            ...
            <div className="p-5 space-y-4">
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Form Description:</span>
                  <span className="font-bold text-slate-800">{selectedEvent.form.description}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Effective Deadline:</span>
                  <span className="font-bold text-slate-900">{selectedEvent.deadlineDateStr}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-500">Taxpayer TIN / RDO:</span>
                  <span className="font-mono text-slate-800">{selectedEvent.clientTin} / {selectedEvent.clientRdo || 'N/A'}</span>
                </div>
                <div className="flex justify-between items-center text-xs">
              ...
              <div className="flex items-center space-x-3">
                <button
                  ...
                  className="flex-1 bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold py-2.5 rounded-xl transition-all flex items-center justify-center space-x-2"
                >
                ...
                {selectedEvent.form.status !== 'Filed' && selectedEvent.form.status !== 'Paid' && (
                  <button
                    ...
                    className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2.5 rounded-xl transition-all flex items-center justify-center space-x-2"
                  >
                    <CheckCircle className="w-3.5 h-3.5" />
